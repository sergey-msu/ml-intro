...
def show_data():
    ...
    train_df = pd.read_csv(utils.PATH.MATERIALS_FILE('final/features.csv'))
    print(train_df.columns.values)
    print(train_df.info())
    print(train_df.shape)

    print(train_df['lobby_type'].value_counts())

    return


def prepare_data():
    train_df = pd.read_csv(utils.PATH.MATERIALS_FILE('final/features.csv'), index_col='match_id')
    test_df  = pd.read_csv(utils.PATH.MATERIALS_FILE('final/features_test.csv'), index_col='match_id')
    ...
    # Всего 12 признаков с пропусками в данных.
    # 'first_blood_*' - событие не успело произойти, 'first_blood_player2' - существенно меньше, чем 'first_blood_player1',
    # что может свидетельствовать о том, что убитый герой не получал урона от первого героя в последние 20 секунд перед смертью
...
def first_approach(X_train, X_test, y_train):
    for n_estimators in [10, 20, 30]:
        ...
        for tr_ids, vl_ids in kf.split(X_train):
            X_tr, X_vl = X_train.values[tr_ids], X_train.values[vl_ids]
